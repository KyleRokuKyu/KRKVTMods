...
public class TerrainTools : ITool
{
	...
	public TerrainTools(Color setColor)
	{
		color = setColor;
	}
	...
	{
		LazyManager<TerrainToolsManager>.Current.ShowSettings(true);
		_selection = new AreaSelection();
	}

	public bool Deactivate(bool soft)
	{
		LazyManager<TerrainToolsManager>.Current.ShowSettings(false);
	...
	private void Fill(Xyz origin, Xyz target, bool preview)
	{
		...
		{
			workerColor = color;
			if (!LazyManager<TerrainToolsManager>.Current.Imperfections)
			{
			...
			{
				Color = color,
				IsLiquid = LazyManager<TerrainToolsManager>.Current.Liquid
			};
